...
const Setting = props => {
    ...
    const setSettings = data => {
        ...
        data && data.map(record => {
            return (
                currentSettings = { ...currentSettings, [record.name]: record.value }
            )
        }); 
    ...
    const getAllSettings = async () => {
        await axios.get(settings.API_URL + '/Setting').then(response => {
            ...
            dispatch({ type: 'SET_SETTINGS', payload: setSettings(response.data.result) });
        });
    }
    ...
    const triggerSearch = () => {        
        axios.post(settings.API_URL + '/Setting/search', { name: search }).then(result => {            
    ...
    const triggerSave = async (e) => {
        ...
        if (name) {
            ...
            if (isEdit) { //update
                ...
                await axios.put(settings.API_URL + '/Setting/' + isEdit, form).then(result => {
                ...
                await axios.post(settings.API_URL + '/Setting/', form).then(result => {
                    if (!result.data.error) {
