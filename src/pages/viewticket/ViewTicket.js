...
//components
import SetStatus from '../ticket/SetStatus';
    ...
    const triggerSubmitReply = async (e) => {
        ...
        if (ticketId && name && email && message) {
            ...
            if (!response?.error) {
                ...
                //show Error
                dispatch({ type: 'SHOW_MODAL', payload: { show: true, message: response?.error } });
            }  
        } else {
            //show Error
            dispatch({ type: 'SHOW_MODAL', payload: { show: true, message: "Required field cannot be empty!"} });
        }        
    }
                ...
                <Col className="col-12 col-md-4">
                    <Card className="shadow position-sticky">
                        ...
                        <ListGroup variant="flush">
                            <ListGroup.Item><b>Ticket #:</b> { selectedData?.data?.ticketNo} </ListGroup.Item>
                            <ListGroup.Item><b>Created:</b> {selectedData?.data?.createdAt} </ListGroup.Item>
                            <ListGroup.Item><b>Category:</b> { selectedData?.data?.category?.name} </ListGroup.Item>
                            <ListGroup.Item><b>Tech:</b> { selectedData?.data?.assignedTech?.name} </ListGroup.Item>
                            <ListGroup.Item><b>Status:</b>  { selectedData?.data?.status?.name} </ListGroup.Item>
                            <ListGroup.Item className="text-center">
                                <Card.Link href="#" onClick={showAssignedTech}>Assign</Card.Link>
                                {/* <Card.Link href="#">Edit</Card.Link>
                                <Card.Link href="#">Delete</Card.Link> */}
                                <Card.Link href="#" onClick={showChangeStatus}>Status</Card.Link>
                            </ListGroup.Item>
                        </ListGroup>
                    </Card>
                </Col>
                <Col className="col-12 col-md-8">
                    <Card className="shadow">
                        <Card.Body>
                            ...
                            <Card.Text className="mb-0 text-primary"> {selectedData?.data?.name} <i className="text-muted">- {selectedData?.data?.email} </i> </Card.Text>
                            <Card.Text className="mb-0 text-muted ">   {new Date(selectedData?.data?.createdAt).toString()} </Card.Text>
                    ...
                    {
                        selectedData?.replies && selectedData?.replies.map(reply => {
                            return (
                                <Replies
                            ...
                            (
                                <Card className="mt-2 shadow">
                                    ...
                                    <Card.Footer className="text-center"> 
                                        <Button type="submit" size="sm" form="frmReply" variant="primary"  >
